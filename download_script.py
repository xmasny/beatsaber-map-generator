...
class DownloadScript:
        ...
        with open("saved_data/usersNew.csv", "r") as users_file:
            ...
            for user in users:
                ...
                for page in range(0, math.ceil(int(int(user[1]) / 20) + 1)):
                    ...
                    print(f"User {user[0]} maps page {page} done")
                    self.terminal_file.write(f"User {user[0]} maps page {page} done\n")
                    self.terminal_file.flush()
        ...
        with open("saved_data/map_info.json", "r") as file:
            ...
            for index, map in enumerate(all_maps):
                all_maps_urls.append(
                    [f"song{index + 1}", map["id"], map["versions"][0]["downloadURL"]]
                )
    ...
    def get_all_maps_from_user(self, start_id=None, end_id=None):
        ...
        for map in all_maps_urls[start_id:end_id]:
            ...
            self.terminal_file.write(f"{map[0]}/{len(all_maps_urls)} downloaded\n")
            self.terminal_file.flush()

    def get_all_zips(self):
        # get all zip files
        for file_name in os.listdir(self.song_folder):
            if file_name.endswith(".zip"):
                self.zip_files.append(file_name)
    ...
    def unzip_all_zips(self):
        ...
        for index, zip_file in enumerate(self.zip_files):
            print(f"Unzipping {zip_file} {index + 1}/{len(self.zip_files)}")
            self.terminal_file.write(
                f"Unzipping {zip_file} {index + 1}/{len(self.zip_files)}\n"
            )
            try:
                ...
                self.terminal_file.write(f"Error occurred in file {index}\n")
                continue
