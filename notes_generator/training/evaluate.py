...
from mir_eval.transcription import (
    match_notes,
    precision_recall_f1_overlap as evaluate_notes,
)
...
def evaluate(model, loader, device, epoch_length_valid, tolerance=0.05):
    metrics = defaultdict(list)
    pbar = tqdm(loader, total=epoch_length_valid, desc="Evaluating")
    for batch in pbar:
...
def predict_test(model: Type[OnsetsBase], loader, result_dict, device):
    for batch in loader:
        _, proba = model.predict_with_probs(batch)  # type: ignore
        target = batch["onset"].long().to(device)
...
def micro_metrics(
    ...
    for prob, tgt in zip(probs, targets):
        ...
        matching = match_notes(
            i_ref, p_ref, i_est, p_est, offset_ratio=None, onset_tolerance=tolerance  # type: ignore
        )
        tp = len(matching)
