...
class SongAndBeatmapsDataset(Dataset):
    ...
    def get_labels(self):
        labels = np.load(join(self.color_notes_folder, "labels.npz")).tolist()
        return labels

    def get_timestamps(self, mel_spectrogram, beats, bpm):
        timestamps = librosa.times_like(
            mel_spectrogram, sr=sample_rate, hop_length=hop_length
        )
    ...
    def __getitem__(self, index):
        ...
        timestamp_features, timestamp_labels = self.get_timestamps(
            mel_spectrogram, beats, bpm
        )
